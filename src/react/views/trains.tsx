...
export const Trains: React.FC = () => {
    const { data: trains } = useAutoRefetch<TrainsDto[], TrainFm[]>(EndpointEnum.TRAIN);
    const { data: trainStations } = useAutoRefetch<TrainStationDto[], TrainStationFm[]>(
        EndpointEnum.TRAIN_STATION
    );

    const [trainStationsStep, setTrainStationsStep] = useState<TrainStationStep>();
    ...
    const handlePrepareTrainStationsStep = useCallback(() => {
        ...
        trains?.forEach((train) => {
            ...
            const previousStationId =
                (nextStationId - 1 + train.stationPlanning.length) % train.stationPlanning.length;
            ...
            {trains && trainStationsStep ? (
                <>
                    {trains.map((train) => {
                        ...
                        const { previousStation, nextStation } = trainStationsStep[train.name];
                        let percentDone = 0;
                        let left = 0;
                        let right = 0;
                        let totalLength = 0;
